    ...
    public class DB2Dialect : SqlDialectBase
    {
        ...
        public override string GetSetSql(string sql, int firstResult, int maxResults, IDictionary<string, object> parameters)
        {
            ...
            var newSql = sql
                ...
                .Insert(selectIndex, string.Format("ROW_NUMBER() OVER(ORDER BY {0}) AS {1}, ", orderByClause[9..], GetColumnName(null, "_ROW_NUMBER", null)));
        ...
        protected virtual IList<string> GetColumnNames(string sql)
        {
            ...
            var columnSql = sql[start..stop].Split(',');
            ...
            foreach (string c in columnSql)
            {
                ...
                if (index > 0)
                {
                    result.Add(c[(index + 4)..].Trim());
                    continue;
                }

                var colParts = c.Split('.');

                result.Add(colParts[^1].Trim());
            }
