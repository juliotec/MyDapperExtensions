...
namespace DapperExtensions.Predicate
{
    ...
    public abstract class BasePredicate : IBasePredicate
    {
        ...
        {
            var map = sqlGenerator.Configuration.GetMap(entityType) ?? throw new NullReferenceException(string.Format("Map was not found for {0}", entityType));
            var propertyMap = map.Properties.SingleOrDefault(p => p.Name.Equals(propertyName, StringComparison.InvariantCultureIgnoreCase)) ?? throw new NullReferenceException(string.Format("{0} was not found for {1}", propertyName, entityType));
